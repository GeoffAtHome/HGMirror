...
const padding = (M: string): string => {
  ...
  // Mを16進数文字列に変換
  const hexStringM: string = Array.from(new TextEncoder().encode(M))
    .map(v => v.toString(16))
    .join('');
  // 余りの0の配列を生成
  const sizeExtra: number =
    (sizeLastBlock - sizeMLengthBuffer - sizeDivision - sizeLastM) * 2;
  const hexExtraString: string = Array(sizeExtra).fill(0).join('');
  // Mのサイズを8bytesで表現する
  const hexSizeM: string = (sizeM * 8).toString(16);
  const hex8bytesLengthM: string = Array(16 - hexSizeM.length)
    .fill(0)
    .concat(hexSizeM)
    .join('');
...
const upperSigma0 = (x: number): number =>
  (rotr(x, 2) ^ rotr(x, 13) ^ rotr(x, 22)) >>> 0;

const upperSigma1 = (x: number): number =>
  (rotr(x, 6) ^ rotr(x, 11) ^ rotr(x, 25)) >>> 0;

const lowerSigma0 = (x: number): number =>
  (rotr(x, 7) ^ rotr(x, 18) ^ shr(x, 3)) >>> 0;

const lowerSigma1 = (x: number): number =>
  (rotr(x, 17) ^ rotr(x, 19) ^ shr(x, 10)) >>> 0;
...
const mapW = (array64: number[], Mi: string[]): number[] => {
  for (let i = 0; i < 64; i++) {
    ...
    const tmp: number =
      lowerSigma1(array64[i - 2]) +
      array64[i - 7] +
      lowerSigma0(array64[i - 15]) +
      array64[i - 16];
...
export const computeHash = (M: string): string => {
  ...
  for (let i = 0; i < dividedM.length; i++) {
    ...
    for (let t = 0; t < 64; t++) {
      const T1: number =
        ((h + upperSigma1(e) + ch(e, f, g) + K[t] + W[t]) & 0xffffffff) >>> 0;
  ...
  for (let e = 0; e < H.length; e++) {
    ...
    if (hashString.length < 8) {
      const extraZeros: string = Array(8 - hashString.length)
        .fill(0)
        .join('');
